...
class Node extends Model
{
    ...
    public function getNodesList()
    {
        try {
            ...
            $res = $this->field('node_id as id,node_name as title,node_pid as pid,node_path,node_icon,add_time,is_menu,sort')
                ->select()->order('sort', 'asc')->toArray();
        }catch (\Exception $e) {
    ...
    public function getRoleMenuMap($roleId)
    {
        try {
            ...
            if (1 == $roleId) {
                ...
                $res = $this->field('node_id as id,node_name as title,node_pid as pid,node_path,node_icon,sort')
                    ->where('is_menu', 2)->select()->order('sort', 'asc')->toArray();
            } else {
                ...
                if (!empty($roleInfo)) {
                    ...
                    $res = $this->field('node_id as id,node_name as title,node_pid as pid,node_path,node_icon')
                        ->whereIn('node_id', $roleInfo['role_node'])->where('is_menu', 2)->select()->order('sort', 'desc')->toArray();
                }
            }
        }catch (\Exception $e) {
            Log::error($e->getMessage());
