...
class AdminUser extends Model
{
    ...
    public function getAdmins($limit, $where)
    {
        ...
        try {
            ...
            $res = $this->field($prefix . 'admin_user.*,' . $prefix . 'role.role_name')->where($where)
                ->leftJoin($prefix . 'role', $prefix . 'admin_user.role_id = ' . $prefix . 'role.role_id')
                ->order('admin_id', 'desc')->paginate($limit);

        }catch (\Exception $e) {
//            OperatorLog::error($e->getMessage());
